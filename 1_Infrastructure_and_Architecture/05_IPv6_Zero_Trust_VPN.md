...
*   **Container Boot Orchestration (`secondary-ipv6.service`):** Runs on boot in CT 111. It applies the static homelab ULA (`fddf::111/64`) and chains immediately into `/usr/local/bin/update-vpn-dns.sh` to derive the dynamic Secondary GUA directly from kernel SLAAC Router Advertisements (RA).
*   **DDNS & Dynamic Route Pinning (`update-vpn-dns.sh`):** A script triggered on boot and hourly via `vpn-ddns.timer` on CT 111. It computes the Secondary Prefix dynamically via hex math on the primary SLAAC address (+1 on the 4th block), cleans up stale GUAs, assigns the new GUA to `eth0`, installs a prioritized default route (`ip -6 route replace default via "${FREEBOX_LLA}" dev "${INTERFACE}" metric 512 src "${NEW_IP}"`) to eliminate RFC 6724 asymmetric source address misrouting, pings the Freebox LLA, and synchronizes the Cloudflare AAAA record (`vpn.paoloferra.com`).
*   **Active WireGuard Watchdog (`wg-watchdog.timer`):** Runs every 3 minutes in CT 111 via `/usr/local/bin/wireguard-watchdog.sh`. It pings the Freebox LLA (`fe80::3a07:16ff:fe21:c7c4%eth0`) to guarantee the Freebox hardware NDP neighbor table stays in `REACHABLE` state, asserts that the prioritized `metric 512` default route is active (triggering `update-vpn-dns.sh` if missing), and asserts `wg0` responsiveness (restarting `wg-quick@wg0` if down).
