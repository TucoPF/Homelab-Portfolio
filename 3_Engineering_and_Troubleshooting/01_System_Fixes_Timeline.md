...
*   **Implementation:**
    1. **Template Setup:** Modified the universal Zsh template in [03_Terminal_Environment_Setup.md](file:///root/Homelab-Portfolio/3_Engineering_and_Troubleshooting/03_Terminal_Environment_Setup.md) to bind `Home` -> `^[[H`, `End` -> `^[[F`, `Delete` -> `^[[3~`, and `clear` -> `^[<`.
    ...
    5. **Documentation:** Appended notes in [07_CSIu_Terminal_Standardization.md](file:///root/Homelab-Portfolio/3_Engineering_and_Troubleshooting/07_CSIu_Terminal_Standardization.md).
...
* **Implementation:**
  1. **Documentation Alignment:** Updated `AGENTS.md`, `/root/Homelab-Portfolio/0_Active_Configurations/01_Homelab_Inventory.md`, and `/root/Homelab-Portfolio/1_Infrastructure_and_Architecture/01_Homelab_Inventory.md`.
...
* **Implementation:**
  1. Created `/etc/modprobe.d/iwlwifi.conf`:
     ```ini
...
### Issue 59: Proxmox Firewall ICMPv6 Drop, NDP Cache Invalidation & CIFS Error -113 (EHOSTUNREACH)
* **Symptoms:** Connecting from laptop T800 via WireGuard VPN or from edge nodes to Samba file storage containers on `skynet` (`fddf::131` / `fddf::133`) failed with Linux kernel error `cifs_mount failed w/return code = -113` (`EHOSTUNREACH`). Inter-container ping from WireGuard gateway (`fddf::111`) to `fddf::131`/`fddf::133` reported `Destination unreachable: Address unreachable`, and the local kernel NDP table (`ip -6 neigh`) showed destination states as `FAILED`.
* **Diagnosis:** 
  1. **Link-Local NDP Filtering:** While PVE firewall had `ndp: 1` enabled in container options, Proxmox's native `ndp` flag strictly permits only raw neighbor solicitations/advertisements (types 135/136) on the direct physical bridge with strict hop-limit constraints, but omits global ICMPv6 control messages (Echo Request/Reply types 128/129, Path MTU Discovery type 2, and router signaling).
  2. **Missing ICMPv6 Rule in Container Firewalls:** Storage containers (`131.fw`, `132.fw`, `133.fw`) defined `GROUP lan-6` (`IN ACCEPT -source fddf::/112`) and `GROUP vpn` (`IN ACCEPT -source fdfd::/112`), but lacked an explicit ICMPv6 acceptance rule. Because incoming Neighbor Solicitations (RFC 4861) originate from Link-Local addresses (`fe80::...`) or unspecified addresses (`::`), they failed to match `fddf::/112` and were silently dropped by Proxmox's default `policy_in: DROP`.
  3. **Conntrack / Warm Cache Illusion:** Outbound packets from the container temporarily populated the remote host's NDP table and created transient `ESTABLISHED,RELATED` Netfilter states, temporarily masking the root cause until cache expiration or host reboot reverted the entry to `FAILED`.
* **Fix Applied:** 
  1. **Cluster-Wide ICMPv6 Unification:** Added `IN ACCEPT -p ipv6-icmp -log nolog` directly into `[group lan-6]` in `/etc/pve/firewall/cluster.fw` without source IP restrictions, allowing uninhibited L2/L3 control plane signaling (NDP, PMTU, ICMPv6 echo) across all nodes calling `lan-6`.
  2. **Firewall Rule Hygiene & Purge:** Removed obsolete/dangling `GROUP ipv6-infra` from cluster rules, purged unused `[group pi]`, and cleaned up orphaned `.fw` files for decommissioned/admin/template nodes without active firewalls.
