...
Since Zsh uses ZLE (Zsh Line Editor) instead of GNU Readline, `/etc/inputrc` is bypassed. The standardized CSI u bindings are injected into the universal Zsh template in `/root/Homelab-Portfolio/3_Engineering_and_Troubleshooting/03_Terminal_Environment_Setup.md` and deployed to the following files:
